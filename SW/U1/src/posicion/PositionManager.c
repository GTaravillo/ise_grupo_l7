...
static void Run(void *argument)
{
  ...
  while (1)
  {
		osThreadFlagsWait(HALL_DETECTED, osFlagsWaitAll, osWaitForever);  // Espera a que haya interrupci�n (cambio de estado de alg�n hall)
    ...
    // Actualiza los valores previos de los buffers despu�s de la lectura
...
static uint8_t mapPinToCasilla(int exp_num, int pin)
{
    // Definimos c�mo se mapea cada expansor a las filas y columnas del tablero
    ...
    // La casilla en formato num�rico es columna * 10 + fila
...
static void ProcessCasillaChanges(void *argument)
{
    ...
    while (1) {
        ...
        // Aqu� procesas el cambio (puedes actualizar el tablero, verificar la jugada, etc.)
        printf("Casilla cambiada: %d\n", casilla_Accionada.casilla);

        // Aqu� puedes agregar la l�gica para actualizar el estado del tablero o realizar alguna acci�n
    }
}
